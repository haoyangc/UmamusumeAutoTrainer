...
def get_operation(ctx: UmamusumeContext) -> TurnOperation | None:
    ...
    trip = False
    if not rest and not ctx.cultivate_detail.turn_info.medic_room_available and (ctx.cultivate_detail.turn_info.date <= 36 and ctx.cultivate_detail.turn_info.motivation_level.value <= 3 and ctx.cultivate_detail.turn_info.remain_stamina < 90 and not support_card_max >= 3
                    ...
                    if j == 3:
                        ...
                        if attr_difference >= incr[j]:
                            ...
                            if cultivate_expect_attribute[j] - turn_expect_attribute[j] > overflow_incr:
                                ...
                                rating_incr += 0.10 * overflow_incr
                            else:
                                #rating_incr += 0.25 * (cultivate_expect_attribute[j] - turn_expect_attribute[j])
                                rating_incr += 0.10 * (cultivate_expect_attribute[j] - turn_expect_attribute[j])
